<x-admin-layout>
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
        <h1 class="text-2xl text-white font-bold">Tambah Data Target Penjualan</h1>
        <br>
        <form action="{{ route('admin.sales-targets.store') }}" method="POST" id="sales-target-form">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" min="0" id="target_amount" name="target_amount" value="{{ old('target_amount') }}"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50"
                    required>
                @error('target_amount')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
        ...
        <x-confirm-create-update-button :name="'confirm-create'" modalForm="sales-target-form"
            confirmMessage="Konfirmasi Tambah Target Penjualan" question="Apakah kamu yakin ingin menyimpan data target penjualan ini?"
